<template>
	<div>
		<section class="Space magic-widget" >
			<div style="width: 100%; height: 10px; background-color: rgb(241, 233, 218);"></div>
		</section>
		<section class="MultipleImages magic-widget" >
			<div class="multiple-images-component top-line">
				<div><a href="javascript:void(0)" >
						<div class="ui-image-component status-show animate animate-end">
							<img class="image" src="https://img-ucdn-static.helijia.com/zmw/upload/magic/images/original_6c465b2d34584f107cea7ee2cc93fc42.jpg!/0/w-540/format-webp">
						</div>
					</a></div>
				<div><a href="javascript:void(0)">
						<div class="ui-image-component status-show animate animate-end">
							<img class="image" src="https://img-ucdn-static.helijia.com/zmw/upload/magic/images/original_9a0ef458654456bcd615e6d60b7e3822.jpg!/0/w-540/format-webp">
						</div>
					</a></div>
			</div>
		</section>
		<section class="Space magic-widget">
			<div style="width: 100%; height: 10px; background-color: rgb(241, 233, 218);"></div>
		</section>
		<section class="ImageLink magic-widget">
			<div class="image-link-component bottom-line">
				<div class="ui-image-component status-show animate animate-end">
					<img class="image" src="https://img-ucdn-static.helijia.com/zmw/upload/magic/images/original_0f32872ca6ffc5a620451b2ce107ffb4.png!/0/w-1080/format-webp">
				</div>
			</div>
		</section>
		<section class="ComplexImages magic-widget">
			<div class="complex-images-component inner-line">
				<div class="left-part">
					<a href="javascript:void(0)" >
						<div class="ui-image-component status-show animate animate-end">
							<img class="image" src="https://img-ucdn-static.helijia.com/zmw/upload/magic/images/original_e4582cd42970a1b473a9675930f295c7.jpg!/0/w-540/format-webp"></div>
					</a></div>
				<div class="right-part">
					<a href="javascript:void(0)" >
						<div class="ui-image-component status-show animate animate-end">
							<img class="image" src="https://img-ucdn-static.helijia.com/zmw/upload/magic/images/original_258d16c8920cc3710a27b5b283c8bc18.jpg!/0/w-540/format-webp">
						</div>
					</a><a href="javascript:void(0)" >
						<div class="ui-image-component status-show animate animate-end">
							<img class="image" src="https://img-ucdn-static.helijia.com/zmw/upload/magic/images/original_d94e3703714054e9b04f65ea37b750d7.jpg!/0/w-540/format-webp"></div>
					</a><a href="javascript:void(0)">
						<div class="ui-image-component status-show animate animate-end">
							<img class="image" src="https://img-ucdn-static.helijia.com/zmw/upload/magic/images/original_ce3e359dbf6fde355303275b1e6fe224.jpg!/0/w-540/format-webp">
						</div>
					</a></div>
			</div>
		</section>
		<section class="Space magic-widget">
			<div style="width: 100%; height: 10px; background-color: rgb(241, 233, 218);"></div>
		</section>
		<section class="ImageLink magic-widget" >
			<div class="image-link-component bottom-line">
				<div class="ui-image-component status-show animate animate-end">
					<img class="image" src="https://img-ucdn-static.helijia.com/zmw/upload/magic/images/original_91a672142198dc6ea2eb479822f3d381.png!/0/w-1080/format-webp">
				</div>
			</div>
		</section>
		<section class="MultipleImages magic-widget" >
			<div class="multiple-images-component inner-line">
				<div><a href="javascript:void(0)" >
						<div class="ui-image-component status-show animate animate-end">
							<img class="image" src="https://img-ucdn-static.helijia.com/zmw/upload/magic/images/original_35202b80f3af877f155822a2c1017d95.jpg!/0/w-360/format-webp">
						</div>
					</a></div>
				<div><a href="javascript:void(0)" >
						<div class="ui-image-component status-show animate animate-end">
							<img class="image" src="https://img-ucdn-static.helijia.com/zmw/upload/magic/images/original_30007f1c82ed00d9931312f7d43973d9.jpg!/0/w-360/format-webp">
						</div>
					</a></div>
				<div><a href="javascript:void(0)" >
						<div class="ui-image-component status-show animate animate-end">
							<img class="image" src="https://img-ucdn-static.helijia.com/zmw/upload/magic/images/original_2be6b4b1088c6a62b635f8acd8e0f208.jpg!/0/w-360/format-webp">
						</div>
					</a></div>
			</div>
		</section>
		<section class="ImageLink magic-widget" >
			<div class="image-link-component top-line"><a href="https://m.helijia.com/search.html?isStudio=1&amp;mixed=true&amp;mode=studio&amp;spm_scope=studio&amp;type=product">
					<div class="ui-image-component status-show animate animate-end">
						<img class="image" src="https://img-ucdn-static.helijia.com/zmw/upload/magic/images/original_d194e969ac39ba9b08da21c1a6e254b5.jpg!/0/w-1080/format-webp">
					</div>
				</a></div>
		</section>
		<section class="Space magic-widget">
			<div style="width: 100%; height: 10px; background-color: rgb(241, 233, 218);"></div>
		</section>
		<section class="Carousel magic-widget" >
			<div class="carousel-component">
				<div class="slick-initialized slick-slider">
					<div class="slick-list">
						<!-- <div class="slick-track" style="opacity: 1; "> -->
							<mt-swipe :auto="4000">
								<mt-swipe-item v-for="a in status[1] ">
									<a href="javascript:void(0)" class="slick-slide slick-cloned"  >
										<div class="ui-image-component status-show animate animate-end" style="height: 100%;">
											<img class="image" :src="a.data.image" style="height: 100%;">
										</div>
									</a>
								</mt-swipe-item>
								<!-- <mt-swipe-item>
									<a href="javascript:void(0)"  class="slick-slide slick-active slick-current"  >
										<div class="ui-image-component status-show animate animate-end" style="height: 100%;">
											<img class="image" src="https://img-ucdn-static.helijia.com/zmw/upload/magic/images/original_830005cd3f0b45dc1b8a0f68d8cefe32.jpg!/0/w-1080/format-webp" style="height: 100%;">
										</div>
									</a>
								</mt-swipe-item> -->
							</mt-swipe>
						<!-- </div> -->
					</div>
				</div>
			</div>
		</section>
		<section class="Space magic-widget">
			<div style="width: 100%; height: 10px; background-color: rgb(241, 233, 218);"></div>
		</section>
	</div>
</template>

<script>
	import {
		Swipe,
		SwipeItem
	} from 'mint-ui'
	import 'mint-ui/lib/style.css'
	export default {
		props: ["status"],
		components: {
			'mt-swipe': Swipe,
			'mt-swipe-item': SwipeItem
		}
	}
</script>

<style scoped>
	.multiple-images-component.top-line {
		position: relative;
	}

	.multiple-images-component {
		-webkit-box-pack: center;
		-ms-flex-pack: center;
		-webkit-justify-content: center;
		-moz-justify-content: center;
		-moz-box-pack: center;
		justify-content: center;
		-webkit-box-direction: normal;
		-webkit-box-orient: horizontal;
		-webkit-flex-direction: row;
		-moz-flex-direction: row;
		-ms-flex-direction: row;
		flex-direction: row;
		-webkit-flex-wrap: nowrap;
		-moz-flex-wrap: nowrap;
		-ms-flex-wrap: none;
		flex-wrap: nowrap;
		-webkit-box-align: center;
		-ms-flex-align: center;
		-webkit-align-items: center;
		-moz-align-items: center;
		-moz-box-align: center;
		align-items: center;
	}

	.multiple-images-component {
		display: -webkit-box;
		display: -webkit-flex;
		display: -moz-flex;
		display: -ms-flexbox;
		display: -moz-box;
		display: flex;
	}

	*,
	*:after,
	*:before {
		-webkit-box-sizing: border-box;
		-moz-box-sizing: border-box;
		box-sizing: border-box;
	}

	@media (-webkit-min-device-pixel-ratio: 2) .multiple-images-component.top-line:before {
		width: 200%;
		-ms-transform: scale(0.5);
		transform: scale(0.5);
		-webkit-transform: scale(0.5);
	}

	.multiple-images-component.top-line:before {
		height: 0;
		border-top: 1px solid #D2C7AB;
		position: absolute;
		content: "";
		left: 0;
		top: 0;
		width: 100%;
		-ms-transform-origin: left top;
		transform-origin: left top;
		-webkit-transform-origin: left top;
	}

	.ui-image-component {
		position: relative;
		overflow: hidden;
	}

	button>*,
	a>* {
		pointer-events: none;
	}

	.ui-image-component .image {
		display: block;
		width: 100%;
	}

	.image-link-component.bottom-line {
		position: relative;
	}

	.complex-images-component {
		display: -webkit-box;
		display: -webkit-flex;
		display: -moz-box;
		display: -ms-flexbox;
		display: flex;
	}

	.complex-images-component.inner-line .left-part a>div {
		position: relative;
	}

	.image-link-component.bottom-line:after {
		height: 0;
		border-bottom: 1px solid #D2C7AB;
		position: absolute;
		content: "";
		left: 0;
		bottom: 0;
		width: 100%;
		-ms-transform-origin: left bottom;
		transform-origin: left bottom;
		-webkit-transform-origin: left bottom;
	}

	.multiple-images-component.inner-line>div:after {
		width: 0;
		border-right: 1px solid #D2C7AB;
		position: absolute;
		content: "";
		right: 0;
		top: 0;
		height: 100%;
		-ms-transform-origin: right top;
		transform-origin: right top;
		-webkit-transform-origin: right top;
	}

	.image-link-component.top-line:before {
		height: 0;
		border-top: 1px solid #D2C7AB;
		position: absolute;
		content: "";
		left: 0;
		top: 0;
		width: 100%;
		-ms-transform-origin: left top;
		transform-origin: left top;
		-webkit-transform-origin: left top;
	}



	@media (-webkit-min-device-pixel-ratio: 3) .complex-images-component.inner-line .left-part a>div:after {
		height: 300%;
		-ms-transform: scale(0.333);
		transform: scale(0.333);
		-webkit-transform: scale(0.333);
	}

	@media (-webkit-min-device-pixel-ratio: 2) .complex-images-component.inner-line .left-part a>div:after {
		height: 200%;
		-ms-transform: scale(0.5);
		transform: scale(0.5);
		-webkit-transform: scale(0.5);
	}

	.complex-images-component.inner-line .left-part a>div:after {
		width: 0;
		border-right: 1px solid #D2C7AB;
		position: absolute;
		content: "";
		right: 0;
		top: 0;
		height: 100%;
		-ms-transform-origin: right top;
		transform-origin: right top;
		-webkit-transform-origin: right top;
	}

	.complex-images-component .left-part {
		-webkit-box-flex: 1;
		-webkit-flex: 1;
		-moz-box-flex: 1;
		-ms-flex: 1;
		flex: 1;
	}

	.complex-images-component .right-part {
		-webkit-box-flex: 1;
		-webkit-flex: 1;
		-moz-box-flex: 1;
		-ms-flex: 1;
		flex: 1;
	}

	.complex-images-component.inner-line .right-part a:first-child>div:before {
		border-top: none;
	}

	.complex-images-component.inner-line .right-part a>div {
		position: relative;
	}

	img {
		border-style: none;
	}

	.multiple-images-component.inner-line>div {
		position: relative;
	}

	.multiple-images-component>div {
		-webkit-box-flex: 1;
		-webkit-flex: 1;
		-moz-box-flex: 1;
		-ms-flex: 1;
		flex: 1;
	}

	.image-link-component.top-line {
		position: relative;
	}

	.image-link-component.top-line:before {
		top: -1px;
	}

	.ui-image-component {
		position: relative;
		/* overflow: hidden; */
	}

	.carousel-component {
		max-height: 12rem;
	}

	.slick-slider {
		position: relative;
		display: block;
		-webkit-box-sizing: border-box;
		-moz-box-sizing: border-box;
		box-sizing: border-box;
		-webkit-user-select: none;
		-moz-user-select: none;
		-ms-user-select: none;
		user-select: none;
		-webkit-touch-callout: none;
		-khtml-user-select: none;
		-ms-touch-action: pan-y;
		touch-action: pan-y;
		-webkit-tap-highlight-color: transparent;
	}

	.slick-list {
		-webkit-transform: translate3d(0, 0, 0);
		-ms-transform: translate3d(0, 0, 0);
		transform: translate3d(0, 0, 0);
		height: 6rem;
	}

	.slick-track:before,
	.slick-track:after {
		display: table;
		content: '';
	}

	.slick-initialized .slick-slide {
		display: block;
	}

	.slick-track {
		position: relative;
		top: 0;
		left: 0;
		display: block;
		margin-left: auto;
		margin-right: auto;
	}

	.slick-slide {
		display: none;
		float: left;
		height: 100%;
		min-height: 1px;
	}

	.slick-slide img {
		display: block;
	}

	.complex-images-component.inner-line .left-part a>div:after {
		width: 0;
		border-right: 1px solid #D2C7AB;
		position: absolute;
		content: "";
		right: 0;
		top: 0;
		height: 100%;
		-ms-transform-origin: right top;
		transform-origin: right top;
		-webkit-transform-origin: right top;
	}

	.complex-images-component.inner-line .right-part a>div:before {
		height: 0;
		border-top: 1px solid #D2C7AB;
		position: absolute;
		content: "";
		left: 0;
		top: 0;
		width: 100%;
		-ms-transform-origin: left top;
		transform-origin: left top;
		-webkit-transform-origin: left top;
	}
</style>
